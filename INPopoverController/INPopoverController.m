...
@interface INPopoverController () <NSFileManagerDelegate>
...
- (void)presentPopoverFromRect:(NSRect)rect inView:(NSView *)positionView preferredArrowDirection:(INPopoverArrowDirection)direction anchorsToPositionView:(BOOL)anchors
{
    ...
    _screenRect.origin = [mainWindow convertPointToScreen:_screenRect.origin]; // Convert window coordinates to screen coordinates
...
- (CGFloat)arrowOffset
{
    return _popoverWindow.frameView.arrowOffset;
}
...
- (void)setArrowSize:(NSSize)arrowSize
{
    ...
    NSRect bounds = [_popoverWindow frame];
    bounds.origin = NSZeroPoint;
    [_popoverWindow.popoverContentView setFrame:[_popoverWindow contentRectForFrameRect:bounds]];
}

- (void)setArrowOffset:(CGFloat)arrowOffset
{
    _popoverWindow.frameView.arrowOffset = arrowOffset;
}
...
- (void)_setInitialPropertyValues
{
    ...
    _popoverWindow = [[INPopoverWindow alloc] initWithContentRect:NSZeroRect styleMask:NSWindowStyleMaskBorderless backing:NSBackingStoreBuffered defer:NO];
    _popoverWindow.appearance = [NSAppearance appearanceNamed:NSAppearanceNameVibrantLight];
...
- (void)_positionViewFrameChanged:(NSNotification *)notification
{
    ...
    _screenRect.origin = [[self.positionView window] convertPointToScreen:_screenRect.origin]; // Convert window coordinates to screen coordinates
