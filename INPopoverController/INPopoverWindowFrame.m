...
- (void)drawRect:(NSRect)dirtyRect
{
    ...
    const CGFloat arrowMidWidth = arrowWidth != 0 ? (arrowWidth / 2.0) : 0;
    ...
    if (self.topHighlightColor) {
        ...
        if (self.arrowDirection == INPopoverArrowDirectionUp) {
            CGFloat width = floor((lineRect.size.width / 2.0) - arrowMidWidth);
            NSRectFill(NSMakeRect(lineRect.origin.x, lineRect.origin.y, width, lineRect.size.height));
            NSRectFill(NSMakeRect(floor(lineRect.origin.x + (lineRect.size.width / 2.0) + arrowMidWidth), lineRect.origin.y, width, lineRect.size.height));
        } else {
            NSRectFill(lineRect);
        }
    }
...
- (NSBezierPath *)_popoverBezierPathWithRect:(NSRect)aRect
{
    ...
    if (self.arrowDirection == INPopoverArrowDirectionDown) {
        ...
        NSPoint points[3];
        points[0] = NSMakePoint(floor(midX - arrowMidWidth) + self.arrowOffset, minY - radius); // Starting point
        points[1] = NSMakePoint(floor(midX) + self.arrowOffset, points[0].y - arrowHeight); // Arrow tip
        points[2] = NSMakePoint(floor(midX + arrowMidWidth + self.arrowOffset), points[0].y); // Ending point
    ...
    if (self.arrowDirection == INPopoverArrowDirectionRight) {
        ...
        NSPoint points[3];
        points[0] = NSMakePoint(maxX + radius + self.arrowOffset, floor(midY - arrowMidWidth));
        points[1] = NSMakePoint(points[0].x + arrowHeight + self.arrowOffset, floor(midY));
        points[2] = NSMakePoint(points[0].x + self.arrowOffset, floor(midY + arrowMidWidth));
        [path appendBezierPathWithPoints:points count:3];
    }
    // Top right corner
    ...
    if (self.arrowDirection == INPopoverArrowDirectionUp) {
        CGFloat midX = NSMidX(drawingRect);
        NSPoint points[3];
        points[0] = NSMakePoint(floor(midX + arrowMidWidth + self.arrowOffset), maxY + radius);
        points[1] = NSMakePoint(floor(midX + self.arrowOffset), points[0].y + arrowHeight);
        points[2] = NSMakePoint(floor(midX - arrowMidWidth + self.arrowOffset), points[0].y);
        [path appendBezierPathWithPoints:points count:3];
    }
    // Top left corner
    ...
    if (self.arrowDirection == INPopoverArrowDirectionLeft) {
        CGFloat midY = NSMidY(drawingRect);
        NSPoint points[3];
        points[0] = NSMakePoint(minX - radius + self.arrowOffset, floor(midY + arrowMidWidth));
        points[1] = NSMakePoint(points[0].x - arrowHeight + self.arrowOffset, floor(midY));
        points[2] = NSMakePoint(points[0].x + self.arrowOffset, floor(midY - arrowMidWidth));
        [path appendBezierPathWithPoints:points count:3];
    }
